...
void __interrupt() isr(void){
    ...
    if (PIR1bits.ADIF) {
        POT2 = ADRESH;//adc_read(); //Le asignamos a número el valor del ADC
        
        PIR1bits.ADIF = 0;
    }
    
    if(INTCONbits.RBIF){
        if(PORTBbits.RB1 == 0){//Incrementa contador
            contador++;
        }
        else if(PORTBbits.RB0 == 0){ //Decrementa contador
            contador--;
        }
    ...
    if(PIR1bits.SSPIF == 1){ //Revisamos si ya hay datos que recibir
        valor = spiRead();
        if (valor == 0) //Si el dato recibido es 0 mandamos el valor del ADC
        {
            spiWrite(POT2);
        }
        if (valor == 1) //Si el dato recibido es 1 mandamos el valor del contador
        {
            spiWrite(contador);
            //__delay_ms(1);
        }
